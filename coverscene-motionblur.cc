	...
	for (int a = -10; a < 10; a++)
		for (int b = -10; b < 10; b++) {
			...
			if ( (centre - vec3(4.0, 0.2, 0.0)).length() > 0.9 ) {
				if (c_mat < 0.8) 		
					list[i ++] = new moving_sphere(centre, centre + vec3(0.0, 0.5 * drand48(), 0.0), 0.0, 1.0, 0.2, new lambertian(vec3(drand48() * drand48(), drand48() * drand48(), drand48() * drand48())));
				else if (c_mat < 0.95) 	
					list[i ++] = new sphere(centre, 0.2, new metal(vec3(0.5 * (1 + drand48()), 0.5 * (1 + drand48()), 0.5 * (1 + drand48())), 0.5 * drand48()));
				else					
...
int main () {
	freopen("coverscene-motionblur.ppm", "w", stdout);
	...
	double aperture = 0.0;
	camera cam(look_from, look_at, vec3(0.0, 1.0, 0.0), 20, double(nx) / double(ny), aperture, dist, 0.0, 1.0);
